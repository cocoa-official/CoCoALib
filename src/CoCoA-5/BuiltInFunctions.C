...
DECLARE_STD_BUILTIN_FUNCTION(insert, 3) {
	...
	intrusive_ptr<LIST> list = runtimeEnv->obtainUnshared<LIST>(ARG(0));
  //	LIST::ContainerType::size_type index = list->checkIndex(shouldBeAnIndex, indexArg.exp);
	LIST::ContainerType::size_type index = list->checkIndexOrEnd(shouldBeAnIndex, indexArg.exp);
...
DECLARE_STD_BUILTIN_FUNCTION(GetErrMesg, 1) {
	const Argument &arg = ARG(0);
	intrusive_ptr<ERROR_MESG> err = runtimeEnv->evalArgAs<ERROR_MESG>(arg);
...
DECLARE_STD_BUILTIN_FUNCTION(VersionInfo, 0) { // AMB
  ...
  rec->setFieldNoCheck("CoCoA_32BIT_LONG", Value::from(BuildInfo::CoCoA_32BIT_LONG_Flag()));
...
DECLARE_STD_BUILTIN_FUNCTION(WordList, 0) { // AMB
  (void)(runtimeEnv); // just to keep compiler quiet
	return Value::from(OnlineHelp::WordList());
}
END_STD_BUILTIN_FUNCTION
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/CoCoA-5/BuiltInFunctions.C,v 1.323 2024/07/02 12:26:36 bigatti Exp $
// $Log: BuiltInFunctions.C,v $
// Revision 1.323  2024/07/02 12:26:36  bigatti
// Summary: fixed insert using new fn checkIndexOrEnd
//
// Revision 1.322  2024/04/12 16:58:42  abbott
// Summary: Changed name of CPP symbol which signifies that longs ae 32 bit
//
// Revision 1.321  2024/03/26 06:59:58  bigatti
// Summary: Added CoCoA_32BIT_Flag() function, and using in for printing in CoCoALib/5
//
// Revision 1.320  2024/03/24 10:55:10  abbott
// Summary: Renamed class ERROR to class ERROR_MESG (redmine 1661)
//
// Revision 1.319  2023/12/10 15:32:18  abbott
// Summary: Added "void use of runtimeEnv" to keep compiler quiet
//
// Revision 1.318  2023/12/01 15:36:28  bigatti
// Summary: added WordList() returning LIST of STRING
//
// Revision 1.317  2023/11/21 22:09:15  abbott
// Summary: Added CompilationPlatform to VersionInfo
//
