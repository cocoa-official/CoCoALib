...
// //  Deliberately skips indexes 1 and 2
...
//           {
//             TargetNumEvalPts = NumEvalPts+3; // if no indexes removed in 3 consecutive iters, we accept the list as "probably correct" (i.e. only very few false positives)
...
//     }
  ...
  std::vector<long> FindCycloFactor(vector<long> CandidateCycloIndexes, RingElem f)
  {
    ...
    while (true)
    {
      ...
      swap(CandidateCycloIndexes, ReducedCandidateList); // really assignment
      VERBOSE(80) << "CandidateCycloIndexes=" << CandidateCycloIndexes << std::endl;
      if (CandidateCycloIndexes.empty())  return CandidateCycloIndexes;
      // advance to next EvalPt
      const long skip = RandomLong(1, 50); for (long i=0; i < skip; ++i)  GotoNextEvalPt(EvalPtNumer, EvalPtDenom);
    }
  }
  ...
  namespace // anonymous -- auxiliaries for CyclotomicFactorIndexes
  {

    // List of all k>2 such that EulerTotient(k) <= d
    // EQUIV list of all indexes k>2 such that deg(cyclo(k)) <= d
  ...
  factorization<RingElem> CyclotomicFactors(ConstRefRingElem f)
  {
    ...
    for (int i=0; i < len(F); ++i)
    {
      ...
      const vector<long> CandidateIndexes = CyclotomicFactorIndexes(F[i]);
      // MAY GIVE FALSE POSITIVE { long sum = 0; for (long k: CandidateIndexes) sum += EulerTotient(k); if (sum == deg(F[i])) { ...???... }}
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-cyclotomic.C,v 1.34 2024/07/03 16:07:44 bigatti Exp $
// $Log: SparsePolyOps-cyclotomic.C,v $
// Revision 1.34  2024/07/03 16:07:44  bigatti
// Summary: Error codes (Redmine #427): changed some string messages into equivalent error codes
//
// Revision 1.33  2024/07/02 09:57:58  bigatti
// Summary: error codes, first batch:
// ReqUnivariate, ReqNonZero, ReqNonZeroGradingDim, ReqNonZeroModulus,  ReqNonZeroRingElem
//
// Revision 1.32  2024/04/12 16:55:52  abbott
// Summary: Changed name of CPP symbol which signifies that longs are 32 bits (redmine 1661/1804)
//
// Revision 1.31  2024/03/25 19:08:52  abbott
// Summary: Changed to ifdef/ifndef
//
// Revision 1.30  2024/03/25 08:04:26  abbott
// Summary: Modifications to work  on MinGW (& 32 bitters) (redmine 1661)
//
// Revision 1.29  2024/01/25 21:06:44  abbott
// Summary: Commented out unused fn CycloCoeffHeightBound_sqfr
//
// Revision 1.28  2023/12/10 15:33:01  abbott
// Summary: Fixed minor bug
//
// Revision 1.27  2023/12/09 20:29:49  abbott
// Summary: Split "eval" fns out of SparsePolyOps-cyclotomic, into SparsePolyOps-eval; added example & doc
//
// Revision 1.26  2023/11/21 22:07:46  abbott
// Summary: Added TEMPORARY EvalUPoly (needs to proerly done)
//
// Revision 1.25  2023/11/20 20:49:15  abbott
// Summary: Exensive changes/improvements.  Fixed bug in BeukersSmyth; new FindCycloIndex; removed some cruft
//
// Revision 1.24  2023/11/08 21:37:19  abbott
// Summary: Allow large index inc cyclotomic if there are at most 3 distinct odd prime factors
//
// Revision 1.23  2023/10/05 18:01:06  abbott
// Summary: Simplified some verbose mesgs (to avoid printing big polys); some tidying
//
// Revision 1.22  2023/07/26 12:02:20  abbott
// Summary: Several minor updates
//
// Revision 1.21  2023/07/03 14:23:34  abbott
// Summary: Changed local var name from OVERFLOW (which is a system macro on Macs) to UPB; anyway a better name
//
